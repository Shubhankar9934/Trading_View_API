        ...
        try:
            ...
            if res:
                ...
                if json_res["m"] == "qsd":
                    ...
                    st.write(f"{symbol} -> {price=}, {change=}, {change_percentage=}, {volume=}")
            else:
...
def get_symbol_id(pair, market):
    ...
    st.write(symbol_id, end="\n\n")
    return symbol_id

def main():
    st.title("TradingView WebSocket Streamlit App Epitome Consultancy")

    pair = st.text_input("Enter pair (e.g., btcusdt):")
    market = st.selectbox("Select market:", ["crypto", "stocks", "forex"])

    if st.button("Start WebSocket Connection"):
        run_websocket(pair, market)

def run_websocket(pair, market):
    try:
        st.text("Connecting to TradingView WebSocket...")
        ...
        st.text("WebSocket connection established. Waiting for data...")
